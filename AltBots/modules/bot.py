...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        jarvis = await e.reply(f"˹𝐅ɪɴᴀʟ ꭙ 𝐒ᴛʀɪᴋᴇ˼⚡🔥")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await jarvis.edit(f"˹𝐅ɪɴᴀʟ ꭙ 𝐒ᴛʀɪᴋᴇ˼\n» `{mp} ᴍꜱ`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`˹𝐅ɪɴᴀʟ ꭙ 𝐒ᴛʀɪᴋᴇ˼⚡🔥 𝗜𝗦 𝗦𝗧𝗔𝗥𝗧𝗜𝗡𝗚.`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"»˹𝐅ɪɴᴀʟ ꭙ 𝐒ᴛʀɪᴋᴇ˼⚡🔥 𝗡𝗘𝗪 𝗦𝗨𝗗𝗢 𝗨𝗦𝗘𝗥 𝗔𝗗𝗗𝗘𝗗__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"˹𝐅ɪɴᴀʟ ꭙ 𝐒ᴛʀɪᴋᴇ˼⚡🔥 𝗦𝗨𝗗𝗢 𝗨𝗦𝗘𝗥. !!")
        else:
            if len(sudousers) > 0:
                newsudo = f"{sudousers} {target}"
            else:
                newsudo = f"{target}"
            await ok.edit(f"» **ɴᴇᴡ ꜱᴜᴅᴏ ᴜꜱᴇʀ**: `{target}`\n» `SUDO ADDED...BOT IS RESTARTING`")
            heroku_var["SUDO_USERS"] = newsudo    
    
    elif event.sender_id in SUDO_USERS:
        await event.reply("»˹𝐅ɪɴᴀʟ ꭙ 𝐒ᴛʀɪᴋᴇ˼⚡🔥 ONLY 𝗢wner Can give 𝗦𝗨𝗗𝗢...")
...
async def removesudo(event):
    if event.sender_id == OWNER_ID:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        sudousers = getenv("SUDO_USERS", default=None)
        ok = await event.reply(f" NIKAL BEE MADARCHODD...")
        target = ""
        if HEROKU_APP_NAME is not None:
            app = Heroku.app(HEROKU_APP_NAME)
        else:
        ...
        await event.reply("𝗢nly 𝗢wner 𝗖an 𝗥emove 𝗦udo 𝗨sers.")
...
async def show_sudo_users(event):
    if event.sender_id == OWNER_ID:
        sudo_users_list = "˹𝐅ɪɴᴀʟ ꭙ 𝐒ᴛʀɪᴋᴇ˼⚡🔥 𝗖𝗨𝗥𝗥𝗘𝗡𝗧 𝗦𝗨𝗗𝗢 𝗨𝗦𝗘𝗥𝗦 𝗟𝗜𝗦𝗧:\n"
        for user_id in SUDO_USERS:
            sudo_users_list += f"- {user_id}\n"
        await event.reply(sudo_users_list)
    else:
        await event.reply("Only 𝗙or ˹𝐅ɪɴᴀʟ ꭙ 𝐒ᴛʀɪᴋᴇ˼⚡🔥 𝗢wner.")
